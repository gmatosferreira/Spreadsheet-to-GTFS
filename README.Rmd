...
- An __optional__ `stops.csv` file, that if provided will be used as the base file for the GTFS stops, using the ids of the stops provided and generating new ones for those that exist in the routes CSV but do not match any;

> __Notice that the `stop_lat` and `stop_lon` values are initialized with a default 0 value for every stop that was not provided in the `stops.txt`. They must be manually editted!__

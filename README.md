...
GTFS should de stored, `folder_output`; - The agency information, which
will be used for the `agency.txt`; - The services dates list, which will
be used for the `calendar.txt`, and for each, the associated `routes`, a
list of the CSV files that contain each route’s stops and times (without
the `.csv` extension).
...
for (i in seq(length(agency$services$service_id))){
  ...
  for(ti in seq( length( agency$services$routes[[i]] ) ) ) {
    route_name = agency$services$routes[[i]][ti]
    ...
    gtfs_stops <<- rbind(gtfs_stops, data.frame(trips_df[c("stop_id", "stop_name")]))
    
    gtfs_stop_times <<- rbind(gtfs_stop_times, trips_df[c("trip_id", "arrival_time", "departure_time", "stop_id", "stop_sequence")])
    print(sprintf(">> Processed %d trips associated to route!", trip_counter))
  }
}
```
    ...
    ## [1] ">> Processed 135 trips associated to route!"
    ...
    ## [1] ">> Processed 78 trips associated to route!"
    ## [1] "Analysing service 2, start 20250101, end 20251231, with 0 routes..."
    ## [1] "!! Ignoring service, as it has no routes associated..."

A final adjustment must be performed over the `gtfs_stops`, as stops are
appended to this list for every route, and we just want one instance of
each. Lets filter it to make sure we have unique stop_ids.

**Notice that the `stop_lat` and `stop_lon` values are initialized with
a default 0 value for every stop. They must be manually editted!**

``` r
# Filter stops to have unique values by ID, and append latitude and longitude values (THEY SHOULD BE MANUALLY EDITTED!)
gtfs_stops <- gtfs_stops %>%
  distinct(stop_id, .keep_all = TRUE)
gtfs_stops[["stop_lat"]] = 0.0
gtfs_stops[["stop_lon"]] = 0.0
...
write.csv(gtfs_trips, file.path(gtfs_folder, "trips.txt"), row.names = FALSE)
